...
///
/// SMART POINTERS: A data structure that contains a pointer, also additional metadata and capabilities.
/// There are many different kinds of smart pointers in rust.
/// The most common ones are: Box, Rc, RefCell, Arc, Mutex, Cell, Cow, etc.

pub fn _smart_pointers() {
    _boxes();
}
...
fn _boxes() {
    ...
    println!(
        "Size of single_box in bytes: {}",
        std::mem::size_of_val(&single_box)
    ); // 8 bytes
}
